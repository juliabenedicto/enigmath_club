...
export default function BmathLanding() {
  const [timeLeft, setTimeLeft] = useState({ days: 0, hours: 0, minutes: 0, seconds: 0 });

  const getNextFridayAtFive = () => {
    const now = new Date();
  ...
  const nextChallengeDate = getNextFridayAtFive();

  useEffect(() => {
    const interval = setInterval(() => {
      const now = new Date();
      const difference = nextChallengeDate.getTime() - now.getTime();
    ...
    <div className="min-h-screen bg-white p-4 flex flex-col items-center justify-start gap-6">
      <header className="w-full flex justify-between items-center p-4 border-b">
        ...
        <div className="text-center">
          <p className="text-sm text-gray-500">El proper repte es desbloqueja en:</p>
          <div className="text-xl font-semibold">
            {timeLeft.days}d {timeLeft.hours}h {timeLeft.minutes}min {timeLeft.seconds}s
          </div>
        </div>
      </header>

      <main className="w-full max-w-xl text-center">
        <h1 className="text-2xl font-bold mb-4">EnigMath Club</h1>
        <p className="text-md text-gray-600 mb-2">Reptes matemàtics per aprendre a casa</p>
        <p className="text-md text-gray-600 mb-4 italic">Motiva’t, diverteix-te i estima les matemàtiques</p>
        <div className="bg-purple-100 p-4 text-left rounded">
